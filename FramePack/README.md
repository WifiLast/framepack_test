...
- `--enable-tensorrt` (or `FRAMEPACK_ENABLE_TENSORRT=1`) enables an experimental TensorRT path for VAE decoding. It requires `torch-tensorrt`, a CUDA-capable GPU, and can be tuned with `FRAMEPACK_TRT_WORKSPACE_MB` / `FRAMEPACK_TRT_MAX_AUX_STREAMS`. The UI exposes a toggle under **Performance & Output** and will fall back automatically if TensorRT compilation fails.
