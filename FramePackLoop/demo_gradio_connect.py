...
def loop_worker(head_video, tail_video, prompt, n_prompt, generation_count, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf, reduce_file_output, without_preview):
    for generation_count_index in range(generation_count):
        ...
        worker(head_video, tail_video, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf, reduce_file_output, without_preview)
    stream.output_queue.push(('end', None))
    
@torch.no_grad()
def worker(head_video_path, tail_video_path, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf, reduce_file_output, without_preview):
    if stream.input_queue.top() == 'end':
        stream.output_queue.push(('end', None))
        return

    #print(head_video_path)
    #print(tail_video_path)

    # 動画と音声を読み込む
    head_video_frames, audio_frames, info = io.read_video(head_video_path, pts_unit='sec')
    tail_video_frames, audio_frames, info = io.read_video(tail_video_path, pts_unit='sec')
    #print(head_video_frames.shape)
    #print(tail_video_frames.shape)
    if reduce_file_output:
        tmp_filename = "system_preview.mp4"

    #print(head_video.shape)
    fps = 30
    head_latent_num = int(round(head_video_frames.shape[0] / 4))
    if head_latent_num < 3:
        stream.output_queue.push(('end', None))
        return
    else:
        head_N = min(16, (head_latent_num - 3))
    tail_latent_num = int(round((tail_video_frames.shape[0]) / 4))

    #print(head_latent_num)
    #print(tail_latent_num)
    
    if tail_latent_num < 3:
        stream.output_queue.push(('end', None))
        return
    else:
        tail_N = min(16, (tail_latent_num - 3))
        
    #print(head_N)
    #print(tail_N)
    interp_head_frame_length = (head_N + 3) * 4
    remain_head_farme_length = max(0, head_video_frames.shape[0] - interp_head_frame_length)
    interp_tail_frame_length = (tail_N + 3)  * 4
    remain_tail_farme_length = max(0, tail_video_frames.shape[0] - interp_tail_frame_length)
    #print(remain_tail_farme_length, remain_head_farme_length)

    head_remain_video, head_video = head_video_frames.split([remain_head_farme_length,interp_head_frame_length], dim=0)
    tail_video, tail_remain_video = tail_video_frames.split([interp_tail_frame_length,remain_tail_farme_length], dim=0)
    
    #print(head_remain_video.shape)
    #print(tail_remain_video.shape)
    ...
    try:
        ...
        #todo head_videoとtail_videoのトリミングをする必要がある。
        ...
        image_for_encoder = head_video[-1,:,:,:].numpy() # head_videoの最後を使う
        #print(image_for_encoder.shape)
        image_encoder_output = hf_clip_vision_encode(image_for_encoder, feature_extractor, image_encoder)
        ...
        #history_latents = tail_video_latent.cpu()
        #print(head_video_latent.shape)
        #print(tail_video_latent.shape)
        head_history_latents = head_video_latent.cpu()
        tail_history_latents = tail_video_latent.cpu()
        dummy_history_latents = torch.zeros(size=(1, 16, 1 + 2 + 16, height // 8, width // 8), dtype=torch.float32).cpu()
        history_pixels = None

        #torch.zeros(size=(1, 16, 1 + 2 + 16, height // 8, width // 8), dtype=torch.float32).cpu()
        history_pixels = None
        ...
        for latent_padding in latent_paddings:
            ...
            #is_first_section = latent_padding == latent_paddings[-1]
            ...
            indices = torch.arange(0, sum([head_N,2,1,latent_padding_size, latent_window_size, 1, 2, tail_N])).unsqueeze(0)
            pre_clean_latent_4x_indices, pre_clean_latent_2x_indices, clean_latent_indices_pre,blank_indices,\
                latent_indices, clean_latent_indices_post, clean_latent_2x_indices, clean_latent_4x_indices = \
            indices.split([head_N,2,1,latent_padding_size, latent_window_size, 1, 2, tail_N], dim=1)
            ...
            clean_latents_4x_pre, clean_latents_2x_pre,clean_latents_pre  = head_history_latents[:, :, -(1 + 2 + head_N):, :, :].split([head_N, 2, 1], dim=2)
            clean_latents_post, clean_latents_2x, clean_latents_4x = tail_history_latents[:, :, :1 + 2 + tail_N, :, :].split([1, 2, tail_N], dim=2)
            ...
            if is_first_section:
                generated_latents = torch.flip(generated_latents,dims=[1,2])

            total_generated_latent_frames += int(generated_latents.shape[2])
            tail_history_latents = torch.cat([generated_latents.to(tail_history_latents), tail_history_latents], dim=2)
            
            #print(tail_history_latents.shape)          
            ...
            if not without_preview:
                ...
                if history_pixels is None:
                    history_pixels = vae_decode(tail_history_latents, vae).cpu()
                else:
                    section_latent_frames = (latent_window_size * 2)
                    overlapped_frames = latent_window_size * 4 - 3

                    current_pixels = vae_decode(tail_history_latents[:, :, :section_latent_frames], vae).cpu()
            ...
            if is_last_section:
                break
        all_latent_section = total_latent_sections
        #print(all_latent_section)
        if not high_vram:
            offload_model_from_device_for_memory_preservation(transformer, target_device=gpu, preserved_memory_gb=8)
            load_model_as_complete(vae, target_device=gpu)
        
        final_latents = tail_history_latents
        ...
        if remain_head_farme_length > 0:
            head_remain_video_pt = head_remain_video.float() / 127.5 - 1
            head_remain_video_pt = head_remain_video_pt.permute(3, 0, 1, 2).unsqueeze(0)
            #print(head_remain_video_pt.shape)
            final_history_pixels = torch.cat([head_remain_video_pt, final_history_pixels],dim =2 )

        if remain_tail_farme_length > 0:
            tail_remain_video_pt = tail_remain_video.float() / 127.5 - 1
            tail_remain_video_pt = tail_remain_video_pt.permute(3, 0, 1, 2).unsqueeze(0)
            final_history_pixels = torch.cat([final_history_pixels, tail_remain_video_pt],dim =2 )

        #print(final_history_pixels.shape)
        output_filename = os.path.join(outputs_folder, f'{job_id}_{total_generated_latent_frames}_{seed}_connect.mp4')
...
def process(head_video, tail_video, prompt, n_prompt, generation_count, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf,progress_preview_option):
    global stream
    assert head_video is not None, 'No head video!'
    assert tail_video is not None, 'No tail video!'
    ...
    async_run(loop_worker, head_video, tail_video, prompt, n_prompt, generation_count, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf, reduce_file_output, without_preview)
...
with block:
        ...
        with gr.Column():
            ...
            with gr.Group():
                ...
                progress_preview_option = gr.Radio(
                    ...
                    value="Without Preview"                        # ←ここでデフォルト選択！
                )
                gr.Markdown("""          
                ...
                total_second_length = gr.Slider(label="Connect Video Length (Section)", minimum=1, maximum=120, value=1, step=1)
    ...
    ips = [head_video, tail_video, prompt, n_prompt, generation_count, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf, progress_preview_option]
